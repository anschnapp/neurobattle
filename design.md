...
- **Evolution:** population of 21 brains per design. Each generation: top 3 elites survive unchanged, each seeds 6 mutated copies of itself (18 total). Mutation is sparse gaussian noise with configurable σ applied to a configurable fraction of weights. No crossover — for small GA-evolved nets it tends to scramble learned features; concentrating offspring around the top 3 lineages preserves dominant behaviors while still exploring locally via mutation. No mutation decay — constant exploration rate, but the player controls how aggressive it is.
...
- **Mutation strength:** configurable (Low/Medium/High/Extreme, default Medium). Controls both the gaussian sigma and the fraction of weights mutated each generation: Low (σ=0.05, 10%), Medium (σ=0.1, 20%), High (σ=0.2, 35%), Extreme (σ=0.4, 50%). Crank this up when populations get stuck in a local optimum (e.g. all bots charging straight forward); dial it down once a workable behavior emerges and you want to fine-tune it.
