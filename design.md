...
- Config panel is split into three columns: left column for spawn actions (spawn bot 1/2/3, destroy old), center column for training config (design selector, enemy/friend sparring type and count, resource drops, spawn distance), right column for all 12 fitness weight sliders. Player navigates with up/down within a column, left/right to switch columns, primary/secondary to adjust values.
...
1. **20 "student" robots** of the currently selected design are spawned in the zone with **random facing angles** (forces sensor-driven navigation, not memorized paths).
...
- **Hit fBase** (default -30): reward/punish landing hits on the friendly base wall
